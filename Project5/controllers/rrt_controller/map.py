...
class Map:
    ...
    def is_inside_obstacles(self, node, use_all_obstacles = False):
        ...
        #####################################################
        #####################################################
        for obstacle in self._obstacles:  # Assuming obstacle as [(x1, y1), (x2, y2)]
        # Check if node is within the bounding box of any obstacle.
            if (obstacle[0][0] <= node.x <= obstacle[1][0]) and (obstacle[0][1] <= node.y <= obstacle[1][1]):
                return True
        return False
    ...
    def node_generator(self):
        ...
        while True:
            ...
            new_node = Node((x, y))
            if not self.is_inside_obstacles(new_node) and self.is_inbound(new_node):
                return new_node
    ...
    def compute_smooth_path(self, path):
        ...
        ############################################################################
        ############################################################################
        smoothed_path = [path[0]]  # Always include the start node.
        i = 0
        while i < len(path) - 1:
            j = len(path) - 1  # Start from the end of the path.
            while j > i + 1:
