...
def RRT(map):
    ...
    while (map.get_num_nodes() < MAX_NODES):
        ...
        while map.get_num_nodes() < MAX_NODES:
            ...
            nearest_node = find_nearest(map.get_nodes(), rand_node)  # You may need to implement this.
            new_node = map.step_from_to(nearest_node, rand_node)

            if not map.is_inside_obstacles(new_node) and map.is_inbound(new_node):
                map.add_node(new_node)
                map.add_path(nearest_node, new_node)  # Assuming this method exists to draw or save the path.
    ...
    # self implementation:
    def find_nearest(nodes, random_node):
