...
Original file is located at
    https://colab.research.google.com/drive/1yOpWHpHQUxd-AwPUjk_PxMb9zUezQqKu
...
class DiscreteDistribution:
  ...
  def sample(self) -> int:
    ...
    ###############################################################################
    pmf_array = self.pmf()
    # create a pmf distribution array  (returns a proability of each category)
    sample_name = np.random.choice(self._names,1, p=pmf_array)
    # choose a singel random sample from the prior_names array via Discrete Distribution of probability
    sampled_index = self.get_name_index(sample_name)
    # return the sample index via helper function
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
...
def get_category_prior():
    ...
    # initalizing category_prior using the DiscreteDistribution class
...
def sample_category():
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
    category_array = get_category_prior()
    sample = category_array.sample()
...
class DiscreteConditional:
  def __init__(self, A_names, B_names, cpt):
    ...
    # what the last line of code does..
    # 1. compute the sum of each row and result in a 1D array (this is a new sum array!)
    # 2. by using [:, np.newaxis] we get a 2D array -> this means we have a array of one column and n rows...
    # Ex. [[1]
          #[2]
          #[3]]
    # 3. then we divide each row elements (of self._cpt) by the sum of each row (sum array elements (n,1))
  ...
  def sample(self, B_index: int) -> int:
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
    #get the row of the CPT corresponding to the given value of B
    row = self._cpt[B_index]
    #sample a value of A based on the cdp distribution
    sampled_name = np.random.choice(self._A_names,1, p=row)
    sampled_index = self.get_A_index(sampled_name)
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
    return sampled_index
    ...
    def __init__(self, B_names, means, sigmas):
      ...
      Constructor
          Parameters:
              ...
              sigmas (list[float]): list of measurement standard deviation given each category; 표준 편차;
      '''
      self._B_names = B_names.copy() #initialize names of B categories
      self._means = means #intialize mean given each category
      self._sigmas = sigmas #standard deviation given each category
...
def get_pCT():
    ...
    ###############################################################################
    prob_distribution = [
        [0.99, 0.01],  # Probability of cardboard
        [0.99, 0.01], # Paper
        [0.1, 0.9],   # Can
...
def get_pDT():
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
     # Initialize the conditional probability table (CPT) with the prior probabilities of camera detection for each trash category
    prob_distribution = [
...
def sample_conductivity(category=None):
    ...
    pCT = get_pCT()
    if category is None:
        priorCategory = get_category_prior()
        category = priorCategory.sample()
...
def sample_detection(category=None):
    ...
    #                             START OF YOUR CODE                              #
    ###############################################################################
    if category is None: category = sample_category()
...
def likelihood_given_detection(detection):
    ...
    likelihoods = np.array([pDT._cpt[category][detection] for category in range(categoryLen)])
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
    return likelihoods
...
def bayes_given_weight(weight):
    ...
    posteriors = [i / sum(unormalized) for i in unormalized]
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
    return posteriors
