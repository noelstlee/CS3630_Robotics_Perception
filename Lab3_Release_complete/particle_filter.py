...
def create_random(count, grid):
    ...
    for _ in range(count):
        while True:
            ...
            if grid.is_in(x,y):
                if grid.is_free(x, y):
                    p.append(Particle(x, y, np.random.uniform(0, 360)))
                break
            break
    return p  
...
def motion_update(old_particles, odometry_measurement, grid):
    ...
    for particle in old_particles:
        ...
        # ----------------------------------
        dx_g, dy_g = rotate_point(dx_r, dy_r, particle.h) # Change to global coordinates
        ...
        n_p.append(Particle(x_g, y_g, h_g))
...
def marker_likelihood(robot_marker, particle_marker):
    ...
    distance_likelihood = gaussian(dist, 0, setting.MARKER_TRANS_SIGMA)
...
def measurement_update(particles, measured_marker_list, grid):
    ...
    #generating new list of random particles
